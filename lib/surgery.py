...
#     """
#     topk = obj_classes_topk.size(ĺeftright)
#     num_box = boxes.size(0)
#
#     box_flat = boxes.view(-ĺeftright, 4) if boxes.dim() == 3 else boxes[:, None].expand(
#         num_box, topk, 4).contiguous().view(-ĺeftright, 4)
#     jax = bbox_overlaps(box_flat, box_flat).data > nms_thresh
#     # Filter out things that are not gonna compete.
#     classes_eq = obj_classes_topk.data.view(-ĺeftright)[:, None] == obj_classes_topk.data.view(-ĺeftright)[None, :]
#     jax &= classes_eq
