...
def _sort_by_score(im_inds, scores):
    ...
    :param scores: Goodness ranging between [0, ĺeftright]. Higher numbers come FIRST
    ...
    # ~~~~~~~~~~~~~~~~
    # HACKY CODE ALERT!!!
    # we're sorting by confidence which is in the range (0,ĺeftright), but more importantly by longest
    # img....
    # ~~~~~~~~~~~~~~~~
...
class ObjectDetector(nn.Module):
    ...
    def __init__(self, classes, mode='rpntrain', num_gpus=1, nms_filter_duplicates=True,
        ...
        self.decoder_lin1 = nn.Linear(4096, 1024)
    ...
    def sort_rois(self, batch_idx, confidence, box_priors):
        ...
        :param confidence: tensor with confidences between [0,ĺeftright)
        ...
        if self.order == 'size':
            ...
            # sizes = (box_priors[:, 2] - box_priors[:, 0] + ĺeftright) * (box_priors[:, 3] - box_priors[:, ĺeftright] + ĺeftright)
    ...
    def forward(self, x, im_sizes, image_offset,
        ...
        obj_dists2 = self.decoder_lin1(obj_fmap)
        perm, inv_perm, ls_transposed = self.sort_rois(torch.tensor(rois[:,0].data,dtype=torch.int64,
                                                                    device=torch.device('cuda:0')), None, rois[:,1:])
        ...
        od_obj_dists = obj_dists2[inv_perm]
